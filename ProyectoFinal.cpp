...
public:
	...
	// Definición de los getters de la clase Persona
	...
	// Getter para obtener el correo electrónico de la persona
	string getCorreoElectronico() const { return correoElectronico; }
	
	// Definición de los setters de la clase Persona
	...
	// Setter para establecer el correo electrónico de la persona
...
private:
	...
	int tamanoActual; // Variable para almacenar el tamaño actual del arreglo
public:
	// Constructor
	GestorLibro() : tamanoActual(0) {
		// Inicializa el tamaño actual en 0 y llena el arreglo de libros con objetos Libro vacíos
		for (int i = 0; i < TAM; ++i) {
			libros[i] = Libro("", "", 0, "", "", 0, ""); // Valores predeterminados para cada libro
		}
	}
	
	// Método para llenar datos aleatorios para los libros
	void LlenarDatos() {
		...
		for (int i = 0; i < TAM; ++i) {
			...
			tamanoActual++; // Incrementa el tamaño actual del arreglo de libros
		}
	}
	
	// Método para imprimir datos de los libros en diferentes formatos
	void Imprimir(string formato) {
		cout << "LISTADO DE LIBROS:" << endl;
		if (formato == "b") { // Formato breve
			for (int i = 0; i < tamanoActual; ++i) {
				// Imprime cada atributo del libro en líneas separadas
	...
	void Buscar(const string& sBusqueda, int index = 0, int contadorCoincidencias = 0) {
		// Si se ha revisado todo el arreglo, muestra el resultado de la búsqueda
		if (index >= tamanoActual) {
			if (contadorCoincidencias == 0) {
				cout << "No se encontraron resultados para la búsqueda." << endl;
			} else {
				cout << "Se encontraron " << contadorCoincidencias << " coincidencias para la búsqueda." << endl;
			}
			return;
		}
		
		// Busca coincidencias en el título del libro
		if (libros[index].getTitulo().find(sBusqueda) != string::npos) {
			cout << "Título: " << libros[index].getTitulo() << endl;
			contadorCoincidencias++;
		}
		
		// Llama recursivamente a la función para buscar en el siguiente libro del arreglo
...
public:
	// Constructor
	GestorUsuario() : tamanoActual(0) {} // Inicializa el tamaño actual en 0
	
	// Método para llenar datos aleatorios para los usuarios
	void LlenarDatos() {
		...
		for (int i = 0; i < TAM; ++i) {
			// Concatenación para simular un correo electrónico
			...
			tamanoActual++; // Incrementa el tamaño actual del arreglo de usuarios
		}
	}
	
	// Método para imprimir datos de los usuarios en diferentes formatos
	void Imprimir(string formato) {
		cout << "LISTADO DE USUARIOS:" << endl;
		if (formato == "b") { // Formato breve
			for (int i = 0; i < tamanoActual; ++i) {
				// Imprime cada atributo del usuario en líneas separadas
	...
	// Método para buscar usuarios por nombre completo
	void Buscar(const string& sBusqueda, int index = 0, int contadorCoincidencias = 0) {
		// Si se ha revisado todo el arreglo, muestra el resultado de la búsqueda
		if (index >= tamanoActual) {
			if (contadorCoincidencias == 0) {
		...
		// Convertir la cadena de búsqueda a minúsculas para hacer la comparación de manera insensible a mayúsculas/minúsculas
		...
		// Convertir el nombre completo del usuario a minúsculas
		...
		// Compara el nombre completo del usuario con la cadena de búsqueda
...
public:
	// Constructor
	GestorBibliotecario() : tamanoActual(0) {} // Inicializa el tamaño actual en 0
	
	// Método para llenar datos aleatorios para los bibliotecarios
	void LlenarDatos() {
		...
		for (int i = 0; i < TAM; ++i) {
			...
			tamanoActual++; // Incrementa el tamaño actual del arreglo de bibliotecarios
		}
	}
	
	// Método para imprimir datos de los bibliotecarios en diferentes formatos
	void Imprimir(string formato) {
		cout << "LISTADO DE BIBLIOTECARIOS:" << endl;
		if (formato == "b") { // Formato breve
			for (int i = 0; i < tamanoActual; ++i) {
				// Imprime cada atributo del bibliotecario en líneas separadas
	...
	// Método para buscar bibliotecarios por nombre completo
	void Buscar(const string& sBusqueda, int index = 0, int contadorCoincidencias = 0) {
		// Si se ha revisado todo el arreglo, muestra el resultado de la búsqueda
		if (index >= tamanoActual) {
			if (contadorCoincidencias == 0) {
		...
		// Convertir el nombre completo del bibliotecario a minúsculas
		string nombreCompletoMinuscula = bibliotecarios[index].getNombre() + " " + bibliotecarios[index].getApellido();
		transform(nombreCompletoMinuscula.begin(), nombreCompletoMinuscula.end(), nombreCompletoMinuscula.begin(), ::tolower);
		
		// Verificar si la cadena de búsqueda está contenida en el nombre completo del bibliotecario
		if (nombreCompletoMinuscula.find(busquedaMinuscula) != string::npos) {
